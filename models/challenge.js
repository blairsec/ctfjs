...
class Challenge extends Model {
    ...
    static async findSerialized(query, options) {
        if (options === undefined) {
            options = {showDisabled: false, includeFlag: false};
        }
        ...
        const challenges = await db
            ...
            .count("submissions.id as solves")
            .from("challenges")
            .where(query)
            .leftJoin("submissions", function () {
                this.on("challenges.id", "submissions.challenge").andOn(
                    "challenges.flag",
                    "submissions.content"
                );
            })
        ...
        for (let c = 0; c < challenges.length; c++) {
            ...
            if (challenges[c].hint === null) {
                delete challenges[c].hint;
            }
            if (options.showDisabled !== true) {
                delete challenges[c].enabled;
            }
            if (options.includeFlag !== true) {
                delete challenges[c].flag;
            }
        }
    ...
    static async findOneSerialized(query, options) {
        ...
        const challenge = await super.findOneSerialized(query, includePrivate);

        if (!options.showDisabled) {
            delete challenge.enabled;
        }
        ...
        challenge.solves = await Submission.findSerialized(
            {challenge: challenge.id},
            {team: true, user: true, challenge: false, solved: true}
        );
