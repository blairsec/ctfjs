...
class Submission extends Model {
    ...
    static async findSerialized(properties, options) {
        ...
        if (options === undefined) {
            options = {team: true, user: true, challenge: true};
        }
        let query = db
            .select("submissions.id", "submissions.created as time")
            .from("submissions")
            .where(properties);
        if (options.challenge) {
            query = query.select(
                "challenges.id as _challenge__id",
                "challenges.title as _challenge__title",
                "challenges.category as _challenge__category",
                "challenges.value as _challenge__value",
                "challenges.author as _challenge__author"
            );
        }
        query = query.join("challenges", function () {
            const on = this.on("challenges.id", "submissions.challenge");
            if (options.solved) {
                on.andOn("challenges.flag", "submissions.content");
            }
        });
        if (options.user) {
            query = query
                .select(
                    "users.id as _user__id",
                    "users.username as _user__username"
                )
                .leftJoin("users", "users.id", "submissions.user");
        }
        if (options.team) {
            query = query
                .select(
                    ...
                    "teams.affiliation as _team__affiliation"
                )
                .leftJoin("teams", "teams.id", "submissions.team");
        }
