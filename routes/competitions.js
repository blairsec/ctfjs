module.exports = function (ctf) {
    ...
    router.get("/", async function (req, res) {
        ...
        const competitions = await Competition.findSerialized({});
        await ctf.emitAfter("getCompetitions", req, {
    ...
    router.get("/:competition", async function (req, res) {
        ...
        const competition = await Competition.findOneSerialized({
            id: req.params.competition,
        });
    ...
    router.post(
        ...
        async function (req, res) {
            if (req.user.admin) {
                ...
                if (typeof req.body.teamSize === "number") {
                    competition.teamSize = req.body.teamSize;
                }
                await competition.save();
                await ctf.emitAfter("createCompetition", req, {
        ...
        async function (req, res) {
            if (req.user.admin) {
                ...
                if (!competition) {
                    return req
                        .status(404)
                        .json({message: "competition_not_found"});
                }

                if (req.body.start && errors.indexOf("start") === -1) {
                    competition.start = new Date(req.body.start);
                }
                if (req.body.end && errors.indexOf("end") === -1) {
                    competition.end = new Date(req.body.end);
                }
                if (req.body.about && errors.indexOf("about") === -1) {
                    competition.about = req.body.about;
                }
                if (req.body.name && errors.indexOf("name") === -1) {
                    competition.name = req.body.name;
                }
                if (req.body.teamSize && errors.indexOf("teamSize") === -1) {
                    competition.teamSize = parseInt(req.body.teamSize);
                }
                await competition.save();
                await ctf.emitAfter("modifyCompetition", req, {
                    competition: competition,
                });
